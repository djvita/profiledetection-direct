...
const char *eyeCascadeFilename1 = "/usr/local/share/OpenCV/haarcascades/haarcascade_eye.xml"; // Basic eye detector for open eyes only.
const char *eyeCascadeFilename2 = "/usr/local/share/OpenCV/haarcascades/haarcascade_eye_tree_eyeglasses.xml"; // Basic eye detector for open eyes if they might wear glasses.
...
//******** Variables to add profile information ***********************
int addedPerson=0;
//**********************************************************************
...
void recognizeAndTrainUsingWebcam(VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
    ...
    //*** Variable used to store the identifier of the driver, in case he is recognized:
    int driverID=-1;
    //*** Variables used to read, modify and store the diferent drivers' preferences:
    fstream preferencesFile;
    ofstream preferencesCurrentDriverFile;
    int personID;
    string personID_string,personName,personPreferences;
    vector<string> personsNamesVector, personsPreferencesVector;
    vector<int> personsIDsVector;

    string preferencesFileName, preferencesCurrentDriverFileName;
    preferencesFileName="./preferences/preferencesFile";
    preferencesCurrentDriverFileName="./preferences/preferencesCurrentDriverFile";


    //*** Variables used to read and write vectors into a file:
    fstream fileFacesData_in_out;
    stringstream facesDataCompleteFileName;
    int sizeMat, sizeInt, *apInt, preprocessedFacesSize,label, faceLabelsSize;
    Mat faceToRead, faceToWrite;
    string facesDataFileName,facesBaseFileName, facesFileExtension, facesFolder;

    sizeMat = sizeof(Mat);
    sizeInt = sizeof(int);

    facesFolder = "./facesData/";
    facesBaseFileName = "face";
    facesFileExtension = "jpeg";

    facesDataFileName="facesData.bin";

    facesDataCompleteFileName<<facesFolder<<facesDataFileName;
    //*** Variables used to read and write vectors to a file ***

    //**** Reading the data of the detected faces *****
    fileFacesData_in_out.open((facesDataCompleteFileName.str()).c_str(), ios::in | ios::binary);


    if(fileFacesData_in_out)
    {
        //Reading the number of preprocessed faces:
        apInt = &(preprocessedFacesSize);
        fileFacesData_in_out.read((char*)apInt,sizeInt);

        cout<<"size:"<<preprocessedFacesSize<<"\n";

        //Reading the labels:
        faceLabelsSize=preprocessedFacesSize;
        for(int i=0; i<faceLabelsSize;i++)
        {
            apInt=&(label);
            fileFacesData_in_out.read((char*)apInt,sizeInt);
        ...
        for(int i=0; i<preprocessedFacesSize;i++)
        {
            stringstream facesCompleteFileName;

            facesCompleteFileName<<facesFolder<<facesBaseFileName<<i<<"."<<facesFileExtension;
            faceToRead = imread(facesCompleteFileName.str(),0); //Downloading image as it is (without changing its channels)

/*            cout<<"file:"<<facesCompleteFileName.str()<<"\n";
            imshow("Cara",faceToRead);
            while(waitKey(0)!='q')
            {
            }
*/

            preprocessedFaces.push_back(faceToRead);
        }
        ...
        for(int i=0;i<faceLabelsSize;i++)
        {
            if(faceLabels[i] > m_numPersons)
            {
                m_numPersons = faceLabels[i]; //The highest number of person in faceLabels
            }
        }
        m_numPersons++; //in case of an empty faceLabels vector, m_numPersons=0



        //Storing the reference to the last face of each person:
        //Initializing the 'm_latestFaces' vector:
        for(int k=0; k<m_numPersons; k++)
        {
        ...
        for(int k=0;k<m_numPersons;k++)
        {
            ...
            //Faces are stored in pairs: one original and the other fliping the original
            //The reference is made to the original face, so m_latestFaces must be decremented:
            m_latestFaces[k]--;
        }

        if(m_numPersons>0)
        {
            //number of the last detected person:
            m_selectedPerson=faceLabels[faceLabelsSize-1];

            m_mode=MODE_TRAINING;
        }

//********************
        cout<<"\nNum persons:"<<m_numPersons<<"\n";
        cout<<"\nselected person:"<<m_selectedPerson<<"\n";
        cout<<"\nlatest_faces:"<<"\n";
        for(int i=0; i<m_latestFaces.size();i++)
        {
            cout<<m_latestFaces[i]<<"\n";
        }
//***************
        fileFacesData_in_out.close();
    }
    else
    {
        cout<<"\nError while opening the file for reading the data of the preprocessed faces\n";
    }
    //*************************************************************************
     ...
     //********** Opening and reading the preferences file *******

    preferencesFile.open(preferencesFileName.c_str(),ios::in);
    ...
    if(preferencesFile.is_open())
    {
        ...
        do{
            ...
            preferencesFile>>personID_string;

            if(!preferencesFile.eof())
            {
                personID=(int)strtol(personID_string.c_str(),NULL,10);

                preferencesFile>>personName;
                preferencesFile>>personPreferences;

                personsIDsVector.push_back(personID);
                personsNamesVector.push_back(personName);
                personsPreferencesVector.push_back(personPreferences);
//**********************************************
                cout<<"\npersonID:"<<personID;
                cout<<"\npersonName:"<<personName;
                cout<<"\npersonPreference:"<<personPreferences<<"\n";
//**********************************************


            }

        }while(!preferencesFile.eof());

        preferencesFile.close();
    }
    else
    {
        cout<<"\nError while opening the file for reading the different drivers' preferences\n";
    }
    //***********************************************************
    ...
    printf("%1.0f ms/frame -> %1.1f FPS\n", deltaTime/1000, 1000000/deltaTime ); //FPS
        ...
        Mat preprocessedFace = preprocessFace::getPreprocessedFace(displayedFrame, faceWidth, faceCascade, eyeCascade1, eyeCascade2, preprocessLeftAndRightSeparately, &faceRect, &leftEye, &rightEye, &searchedLeftEye, &searchedRightEye);
            ...
            if (gotFaceAndEyes) {
                ...
                if ((imageDiff > CHANGE_IN_IMAGE_FOR_COLLECTION) && (timeDiff_seconds > CHANGE_IN_SECONDS_FOR_COLLECTION)) {
                    ...
                    if(addedPerson)
                    {
                        ...
                        for(int i=0; i<personsIDsVector.size(); i++)
                        {
                            if(m_selectedPerson==personsIDsVector[i])
                            {
                                index=i;
                                break;
                            }
                        ...
                        //Asking for the new person's data and preferences:
                        personID=m_selectedPerson;
                        personName="";
                        personPreferences="";


                        cout<<"\n**** Setting profile information for person "<<personID<<"****\n";
                        cout<<"     Enter the persons' name (without spaces): ";
                        cin>>personName;
                        cout<<"     Enter the prefered profile (without spaces): ";
                        cin>>personPreferences;
                        cout<<"*************************************\n";

                        if(index!=-1)
                        {
                            //Adding a new person with an existing ID:
                            personsIDsVector[index]=personID;
                            personsNamesVector[index]=personName;
                            personsPreferencesVector[index]=personPreferences;
                        }
                        else
                        {
                            //Adding a new person with a non existing ID:
                            personsIDsVector.push_back(personID);
                            personsNamesVector.push_back(personName);
                            personsPreferencesVector.push_back(personPreferences);
                        }
...
//    vector<int> personsIDsVector;
            ...
            if (gotFaceAndEyes && (preprocessedFaces.size() > 0) && (preprocessedFaces.size() == faceLabels.size())) {
                ...
                if (similarity < UNKNOWN_PERSON_THRESHOLD) {
                    ...
                    //***** Obtaining the recognized driver's ID ***********
                    driverID=identity;
                    //******************************************************
                }
                else {
            ...
            //***** Initializing the recognized driver's ID ***********
            driverID=-1;
            //******************************************************

            //*** Deleting all image files (the binary datafile will be updated at the end)***
            for(int i=0; i<preprocessedFacesSize;i++)
            {
                stringstream facesCompleteFileName;
                facesCompleteFileName<<facesFolder<<facesBaseFileName<<i<<"."<<facesFileExtension;

                remove((facesCompleteFileName.str()).c_str());
            }
            //****************************

            //*** Deleting the preferences file ***********************
            remove(preferencesFileName.c_str());
            //*********************************************************

            //****************************
            //Clearing the vectors of preferences:
            personsIDsVector.clear();
            personsNamesVector.clear();
            personsPreferencesVector.clear();
            //****************************
        ...
        if (keypress == VK_ESCAPE) {   // Escape Key
            ...
            //**** Writing the data of the detected faces: *****
            //opening the file to write:
            preprocessedFacesSize = preprocessedFaces.size();

            fileFacesData_in_out.open((facesDataCompleteFileName.str()).c_str(), ios::out | ios::binary | ios::trunc);

            if(fileFacesData_in_out)
            {
                //writing the number of preprocessed faces:
                apInt = &(preprocessedFacesSize);
                fileFacesData_in_out.write((char*)apInt,sizeInt);

                //writing the face labels:
            ...
            {
                cout<<"\nError while opening the file for storing the data of the preprocessed faces\n";
            }


            //*** Writing the images of the preprocessed faces into files ****
            for(int i=0; i<preprocessedFacesSize;i++)
            {
                stringstream facesCompleteFileName;

                faceToWrite=preprocessedFaces[i];

                facesCompleteFileName<<facesFolder<<facesBaseFileName<<i<<"."<<facesFileExtension;

                /*
                cout<<"i="<<i<<"\n";
                cout<<"\nfile name:"<<facesCompleteFileName.str()<<"\n";
                */

                imwrite(facesCompleteFileName.str(),faceToWrite);

            }
            //****************************************************************


            //****** Writing the changes made to the different drivers' preferences ****
            preferencesFile.open(preferencesFileName.c_str(),ios::out | ios::trunc);

            if(preferencesFile.is_open())
            {
                for(int i=0; i<personsIDsVector.size(); i++)
                {
                    preferencesFile<<personsIDsVector[i];
                    preferencesFile<<" ";
                    preferencesFile<<personsNamesVector[i];
                    preferencesFile<<" ";
                    preferencesFile<<personsPreferencesVector[i];
                    preferencesFile<<"\n";
                }
            ...
            {
                cout<<"\nError while opening the file for writing the different drivers' preferences\n";
            }


            //Write the preferences of the driver in a file:

            preferencesCurrentDriverFile.open(preferencesCurrentDriverFileName.c_str(),ios::out | ios::trunc);

            if(preferencesCurrentDriverFile.is_open())
            {
                for(int i=0; i<personsIDsVector.size(); i++)
                {
                    if(personsIDsVector[i]==driverID)
                    {
                      //  preferencesCurrentDriverFile<<driverID;
                      //  preferencesCurrentDriverFile<<" ";
                        preferencesCurrentDriverFile<<personsNamesVector[i];
                        preferencesCurrentDriverFile<<" ";
                        preferencesCurrentDriverFile<<personsPreferencesVector[i];
                        break;
                    }
                }


                preferencesCurrentDriverFile.close();
            }
            else
            {
                cout<<"\nError while opening the file for writing the recognized driver's preferences\n";
            }
