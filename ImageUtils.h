...
public:
    ...
    static inline int roundFloat(float f);
    ...
    static void setGraphColor(int index DEFAULT(0));
    // Specify the exact color that the next graph should be drawn as.
    static void setCustomGraphColor(int R, int B, int G);
    ...
    // ref: "http://cs.haifa.ac.il/hagit/courses/ist/Lectures/Demos/ColorApplet2/t_convert.html"
    static inline void convertPixelRGBtoHSV_256(int bR, int bG, int bB, int &bH, int &bS, int &bV);
    ...
    // ref: "http://cs.haifa.ac.il/hagit/courses/ist/Lectures/Demos/ColorApplet2/t_convert.html"
    static inline void convertPixelHSVtoRGB_256(int bH, int bS, int bV, int &bR, int &bG, int &bB);
    ...
    static void convertPixelRGBtoHSV_180(int bR, int bG, int bB, int &bH, int &bS, int &bV);
    ...
    static void convertPixelHSVtoRGB_180(int bH, int bS, int bV, int &bR, int &bG, int &bB);
    ...
    static float scaleValueF(float p, float s, float maxVal);
    #define scaleValueFI(p, s, maxVal) scaleValueF(p, s, (float)maxVal)
    // Return (p * s), to a maximum value of maxVal
    static int scaleValueI(int p, float s, int maxVal);
    ...
    static float findDistanceBetweenPointsF(const CvPoint2D32f p1, const CvPoint2D32f p2);
    // Calculate the distance between the 2 given (integer) points.
    static float findDistanceBetweenPointsI(const CvPoint p1, const CvPoint p2);

    // Calculate the angle between the 2 given integer points (in degrees).
    static float findAngleBetweenPointsF(const CvPoint2D32f p1, const CvPoint2D32f p2);
    // Calculate the angle between the 2 given floating-point points (in degrees).
    static float findAngleBetweenPointsI(const CvPoint p1, const CvPoint p2);

    // Draw a crossbar at the given position.
    static void drawCross(IplImage *img, const CvPoint pt, int radius, const CvScalar color );

    // Print the label and then the rect to the console for easy debugging
    static void printPoint(const CvPoint pt, const char *label);
    // Print the label and then the rect to the console for easy debugging
    static void printPointF(const CvPoint2D32f pt, const char *label);
    ...
    static void drawRect(IplImage *img, const CvRect rect, const CvScalar color DEFAULT(CV_RGB(220,0,0)));
    // Draw a filled rectangle around the given object.
    static void drawRectFilled(IplImage *img, const CvRect rect, const CvScalar color DEFAULT(CV_RGB(220,0,0)));

    // Print the label and then the rectangle information to the console for easy debugging
    static void printRect(const CvRect rect, const char *label DEFAULT(0));
    ...
    static void printImagePixels(const IplImage *image, const char *label DEFAULT(0), int maxElements DEFAULT(300));
    // Print the label and then contents of a cv::Mat from the C++ interface (using "LOG()") for easy debugging.
    static void printMat(const cv::Mat M, const char *label DEFAULT(0), int maxElements DEFAULT(300));
    // Print the label and then contents of the Matrix (using "LOG()") for easy debugging. maxElements of 0 will print all the data.
    static void printMatrix(const CvMat *M, const char *label DEFAULT(0), int maxElements DEFAULT(300));
    // Print the label and info of a cv::Mat from the C++ interface (using "LOG()") for easy debugging.
    static void printMatInfo(const cv::Mat M, const char *label DEFAULT(0));
    ...
    static void printPoint32f(const CvPoint2D32f pt, const char *label DEFAULT(0));
    ...
    static void printLine(const CvPoint ptA, const CvPoint ptB, const char *label DEFAULT(0));
    // Just for debugging float images & matrices.
    static void printDataRange(const CvArr *src, const char *msg);
